...
if (c != null) {
    ...
    xhr.open("GET", API_URL+"query=get_item&item_id=" + c, true);
    ...
    xhr.onload = (e) => {
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                ...
                document.querySelector(".item_des_text").innerHTML = result.des;
...
function item_added_modal() {
   ...
   <div class='modal_title'>Товар добавлен в корзину!<br></div>
   <div class='close_modal'><a href='cart' style='color: #66BFFF;'><button class='modal_close_btn''>Перейти в корзину</button></a></div>
