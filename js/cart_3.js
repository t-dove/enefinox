...
if (cart_items != null && cart_items.trim() != "") {
    ...
    xhr.open("GET", API_URL+"query=get_cart_items&item_ids=" + cart_items, true);
...
function send_order() {
   ...
   `;
    ...
    function checkCartModalActive() {
        let check_1 = document.querySelector(".user_name").value.trim();
        let check_2 = document.querySelector(".user_phone").value.trim();
        ...
        document.querySelector(".order_btn").disabled = !(check_1 != "" && check_2 != "" && check_3 != ""  && check_5);
    }
}
...
function send() {
    ...
    let cart_items = getCookie("saved_cart_items");
    const xhr = new XMLHttpRequest();
    xhr.open("POST", API_URL+"query=send_order&item_ids=" + cart_items, true);
    xhr.responseType = "json";
